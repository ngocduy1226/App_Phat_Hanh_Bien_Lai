...
class NumberToVietnamese {
  ...
  static String convert(int number) {
    ...
    // remove extra spaces!
    result = (result.replaceAll(RegExp('\\s+'), ' ').replaceAll('\\b\\s{2,}\\b', ' ')).trim();
    if (result.contains("-năm"))
    {
      result = result.replaceAll("-năm", " lăm");
    }
    if (result.contains("-một"))
    {
      result = result.replaceAll("-một", " mốt");
    }
    if (result.contains("-"))
    {
